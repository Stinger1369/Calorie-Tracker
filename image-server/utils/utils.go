...
// IsCorruptedImage vérifie si une image est corrompue en tentant de la décoder comme un JPEG.
func IsCorruptedImage(filePath string) bool {
	log.Printf("Checking if image is corrupted: %s", filePath)
	...
	// Essayez de décoder l'image en tant que JPEG pour vérifier son intégrité
	_, err = jpeg.Decode(file)
	if err != nil {
		log.Printf("File is corrupted or not a valid JPEG: %v", err)
		return true
	}

	log.Printf("File is a valid JPEG: %s", filePath)
	return false
}
...
func CheckImageForNSFW(filePath string) (bool, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "Attempting to register factory for plugin cuDNN") {
			log.Printf("[%s] CUDNN registration error: %v", ErrCudnnFactoryError, err)
