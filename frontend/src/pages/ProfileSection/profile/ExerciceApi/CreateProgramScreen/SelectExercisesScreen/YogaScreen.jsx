...
import { fetchUserInfoWithFields } from "../../../../../../redux/features/user/userSlice";  // Importation correcte
...
const YogaScreen = ({ navigation, route }) => {
  const {
    programName = "", // Valeurs par défaut pour éviter les erreurs si les paramètres sont absents
    durationMonths = 0,
    sessionsPerWeek = 0,
    currentSession = 1,
    totalSessions = 0,
    sessions = []
  } = route?.params || {};  // Utilisation d'un objet vide si route.params est undefined
  ...
  // Fetch exercises for "Yoga"
  useEffect(() => {
    setLoadingExercises(true);
    if (!cachedExercises["Yoga"]) {
      dispatch(fetchExercisesByMuscleGroup("Postures-de-yoga"))
        .then((res) => {
      ...
      setExercises(cachedExercises["Yoga"]);
      setLoadingExercises(false);
    }
  }, [dispatch]);

  // Fetch like/unlike status for each exercise
  useEffect(() => {
    if (exercisesData && userId && gender) {
      ...
      Promise.all(fetchStatuses).then((updatedExercises) => {
        ...
        setCachedExercises(prevCache => ({
          ...prevCache,
          Yoga: updatedExercises,  // Cache exercises with like/unlike status
        }));
      }).catch((error) => {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Sélectionnez vos exercices de Yoga</Text>
      {loadingExercises ? (
